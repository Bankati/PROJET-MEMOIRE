...
const initialsFromName = ({
  ...
  const words: string[] = fullName.trim().split(" ").filter((word) => word.length > 0);
      ...
      <div className="mx-auto flex w-full max-w-[1460px] gap-4 px-4 py-4 sm:px-6">
        <div className="flex-1 space-y-5">
          <header className="rounded-[26px] border border-zinc-200/70 bg-white px-5 py-4 shadow-[0_10px_28px_-18px_rgba(15,23,42,0.38)] dark:border-white/10 dark:bg-gradient-to-r dark:from-[#122743] dark:via-[#17304f] dark:to-[#2a1435] dark:shadow-2xl dark:shadow-black/25">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div className="space-y-2">
                <p className="inline-flex items-center gap-2 rounded-full border border-lbs-blue/20 bg-lbs-blue/5 px-3 py-1 text-xs text-lbs-blue dark:border-white/15 dark:bg-white/8 dark:text-zinc-100">
                  {roleIconMap[roleLabel]}
                  {roleLabel}
                </p>
                <h1 className="text-2xl font-semibold tracking-tight text-zinc-900 sm:text-3xl dark:text-white">{title}</h1>
          ...
          <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {metrics.map((metric) => (
              <Card key={metric.label} className="rounded-2xl border-zinc-200/90 bg-white shadow-[0_8px_22px_-18px_rgba(15,23,42,0.5)] dark:border-white/10 dark:bg-white/5 dark:shadow-none">
                <CardHeader className="pb-2">
                  <div className="flex items-start justify-between gap-2">
                    <CardDescription className="text-zinc-600 dark:text-zinc-300">{metric.label}</CardDescription>
                    {metric.icon ? (
                  ...
                  <CardTitle className="mt-2 text-2xl text-zinc-900 dark:text-white">{metric.value}</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <Card className="rounded-2xl border-zinc-200/90 bg-white shadow-[0_10px_26px_-18px_rgba(15,23,42,0.55)] dark:border-white/10 dark:bg-white/5 dark:shadow-none">
              <CardHeader>
                <CardTitle className="inline-flex items-center gap-2 text-zinc-900 dark:text-white">
                  <BarChart3 className="size-5 text-lbs-blue" />
        ...
        <aside className="sticky top-4 hidden h-fit w-[96px] shrink-0 rounded-[26px] border border-zinc-200/90 bg-white p-3 shadow-[0_10px_28px_-18px_rgba(15,23,42,0.5)] dark:border-white/10 dark:bg-gradient-to-b dark:from-[#152c4c] dark:to-[#0f2139] dark:shadow-2xl dark:shadow-black/30 xl:block">
          <div className="mb-4 grid place-items-center rounded-2xl border border-zinc-200 bg-[#f8fafc] px-2 py-3 text-center dark:border-white/10 dark:bg-white/8">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-300">Menu</p>
          </div>
          <div className="space-y-2">
            <button type="button" className="group flex w-full flex-col items-center gap-1 rounded-2xl border border-zinc-200 bg-[#eef3ff] py-2 text-[#3e5ecf] transition hover:bg-[#e8eeff] dark:border-white/10 dark:bg-white/10 dark:text-zinc-200 dark:hover:bg-white/15">
              <LayoutDashboard className="size-4" />
              <span className="text-[10px]">Vue</span>
            </button>
            <button type="button" className="group flex w-full flex-col items-center gap-1 rounded-2xl border border-zinc-200 bg-transparent py-2 text-zinc-600 transition hover:bg-zinc-100 dark:border-white/10 dark:text-zinc-300 dark:hover:bg-white/10">
