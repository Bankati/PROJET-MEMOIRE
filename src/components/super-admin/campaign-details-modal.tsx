...
export const CampaignDetailsModal = ({ campaigns }: CampaignDetailsModalProps): React.JSX.Element => {
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {campaigns.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-zinc-400">Aucune campagne.</td>
              </tr>
            ) : (
              campaigns.map((c) => (
                <tr key={c.id} className="border-b border-zinc-100 transition hover:bg-zinc-50/50 dark:border-white/5 dark:hover:bg-white/5">
                  ...
                  <td className="px-4 py-3 text-right">
                    <button
                      ...
                      className="rounded-lg border border-zinc-200 px-3 py-1.5 text-xs font-medium text-zinc-600 transition hover:border-lbs-blue hover:text-lbs-blue dark:border-white/15 dark:text-zinc-300 dark:hover:border-blue-400 dark:hover:text-blue-300"
                    >
                      Détails
                    </button>
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-lg rounded-2xl border border-zinc-200/70 bg-white p-6 shadow-xl dark:border-white/10 dark:bg-[#1a2332]">
            <div className="mb-4 flex items-start justify-between">
              <div>
                <h2 className="text-lg font-bold text-zinc-900 dark:text-white">{selected.title}</h2>
                <p className="text-sm text-zinc-500 dark:text-zinc-400">Admin : {selected.adminName}</p>
              </div>
              <button
                type="button"
                onClick={() => setSelected(null)}
            ...
            <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
                { label: "Contacts", value: selected.totalContacts, color: "text-blue-600 dark:text-blue-300" },
                { label: "Appels", value: selected.totalCalls, color: "text-violet-600 dark:text-violet-300" },
                { label: "Intéressés", value: selected.interestedCount, color: "text-emerald-600 dark:text-emerald-300" },
                { label: "Faux n°", value: selected.falseNumbers, color: "text-rose-600 dark:text-rose-300" },
              ].map((s) => (
                <div key={s.label} className="rounded-xl bg-zinc-50 p-3 text-center dark:bg-white/5">
                ...
                <div className="space-y-2">
                  {selected.topAgents.map((a, idx) => (
                    <div key={a.agentName} className="flex items-center gap-3">
                      ...
                      <p className="flex-1 text-sm font-medium text-zinc-800 dark:text-white">{a.agentName}</p>
                      <span className="text-sm text-zinc-500 dark:text-zinc-400">{a.totalCalls} appels</span>
                      <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300">{a.whatsappCount} WA</span>
                    </div>
                  ))}
                </div>
              )}
