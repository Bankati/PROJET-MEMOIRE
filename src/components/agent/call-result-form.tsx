...
const outcomeOptions: readonly { value: CallOutcome; label: string; icon: React.ReactNode; color: string }[] = [
  { value: "interested", label: "Intéressé", icon: <ThumbsUp className="size-4" />, color: "emerald" },
  { value: "not_interested", label: "Pas intéressé", icon: <ThumbsDown className="size-4" />, color: "rose" },
  { value: "callback", label: "Rappeler", icon: <Clock className="size-4" />, color: "amber" },
  { value: "no_answer", label: "Pas de réponse", icon: <PhoneOff className="size-4" />, color: "zinc" },
  { value: "false_number", label: "Faux numéro", icon: <PhoneMissed className="size-4" />, color: "red" },
  { value: "whatsapp_follow_up", label: "Suivi WhatsApp", icon: <MessageCircle className="size-4" />, color: "green" },
...
export const CallResultForm = ({
  ...
  return (
    <form action={handleSubmit} className="space-y-4">
      ...
      <input type="hidden" name="isWhatsappRedirected" value={showWhatsappOption ? "true" : "false"} />
        ...
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
          {outcomeOptions.map((option) => {
            ...
            const colorClasses: Record<string, string> = {
              emerald: isSelected ? "border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300" : "",
              rose: isSelected ? "border-rose-500 bg-rose-50 text-rose-700 dark:bg-rose-500/15 dark:text-rose-300" : "",
              amber: isSelected ? "border-amber-500 bg-amber-50 text-amber-700 dark:bg-amber-500/15 dark:text-amber-300" : "",
              zinc: isSelected ? "border-zinc-500 bg-zinc-100 text-zinc-700 dark:bg-zinc-500/15 dark:text-zinc-300" : "",
              red: isSelected ? "border-red-500 bg-red-50 text-red-700 dark:bg-red-500/15 dark:text-red-300" : "",
              green: isSelected ? "border-green-500 bg-green-50 text-green-700 dark:bg-green-500/15 dark:text-green-300" : "",
              blue: isSelected ? "border-blue-500 bg-blue-50 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300" : "",
            };
            return (
              <button
                key={option.value}
      ...
      <div>
        ...
        <textarea
          ...
          className="w-full resize-none rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-800 outline-none transition focus:border-lbs-blue focus:ring-2 focus:ring-lbs-blue/20 dark:border-white/15 dark:bg-[#0f1729] dark:text-white"
        />
      </div>
