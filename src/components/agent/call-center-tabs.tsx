...
export const CallCenterTabs = ({
  ...
  const tabs: readonly { id: Tab; label: string; icon: React.ReactNode; hasContent: boolean }[] = [
    { id: 'script', label: 'Script', icon: <ScrollText className="size-4" />, hasContent: false },
    { id: 'ia', label: 'IA', icon: <Bot className="size-4" />, hasContent: false },
    { id: 'docs', label: 'Docs', icon: <FileText className="size-4" />, hasContent: hasPdf },
  ]
      ...
      <div className="flex border-b border-zinc-200 dark:border-white/10">
        {tabs.map((tab) => (
          <button
            ...
            title={
              tab.id === 'docs' && hasPdf
                ? 'Un document PDF est disponible pour cette campagne'
                : undefined
            }
            className={`relative flex flex-1 items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition-all ${
              activeTab === tab.id
          ...
          <div>
            ...
            {hasPdf ? (
              <button
                ...
                className="text-lbs-blue mt-3 flex items-center gap-1.5 text-xs font-medium hover:underline dark:text-blue-300"
              >
                <FileText className="size-3.5" />
                Un document PDF est aussi disponible pour cette campagne — voir l&apos;onglet Docs
              </button>
            ) : null}
          </div>
        ) : null}
