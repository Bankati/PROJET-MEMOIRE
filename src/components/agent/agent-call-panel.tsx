...
import { Delete, MessageCircle } from "lucide-react";

type CallOutcome = "interested" | "not_interested" | "callback" | "no_answer" | "false_number" | "whatsapp_follow_up" | "other";
...
export const AgentCallPanel = ({
  ...
  const handleSubmit = async (formData: FormData): Promise<void> => {
    ...
    try { await submitAction(formData); } finally { setIsSubmitting(false); }
  };
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-2xl border border-zinc-200/70 bg-white shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
        <div className="border-b border-zinc-100 px-5 py-3 dark:border-white/10">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Résultat de l&apos;appel</p>
        </div>
        <form action={handleSubmit} className="space-y-4 p-5">
          ...
          <div className="space-y-1">
            ...
            <input
              type="datetime-local"
              name="callbackAt"
              className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-800 outline-none transition focus:border-lbs-blue focus:ring-2 focus:ring-lbs-blue/20 dark:border-white/10 dark:bg-white/5 dark:text-zinc-100"
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <select
              ...
              onChange={(e) => setSelectedOutcome(e.target.value as CallOutcome || null)}
              className="w-full rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-800 outline-none transition focus:border-lbs-blue focus:ring-2 focus:ring-lbs-blue/20 dark:border-white/10 dark:bg-white/5 dark:text-zinc-100"
            >
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              className="w-full resize-none rounded-xl border border-zinc-200 bg-zinc-50 px-3 py-2.5 text-sm text-zinc-800 outline-none transition focus:border-lbs-blue focus:ring-2 focus:ring-lbs-blue/20 dark:border-white/10 dark:bg-white/5 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-600"
            />
          </div>
          ...
          <button
            ...
            className="w-full rounded-xl bg-zinc-900 px-4 py-3 text-sm font-semibold uppercase tracking-wide text-white transition hover:bg-zinc-800 disabled:cursor-not-allowed disabled:opacity-40 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-100"
          >
      ...
      <div className="rounded-2xl border border-zinc-200/70 bg-white shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
        <div className="flex items-center justify-between border-b border-zinc-100 px-5 py-3 dark:border-white/10">
          <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Composer</p>
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-between rounded-xl border border-zinc-200 bg-zinc-50 px-4 py-2.5 dark:border-white/10 dark:bg-white/5">
            <p className="font-mono text-sm font-medium tracking-wider text-zinc-800 dark:text-white">{dialInput}</p>
            <button
              type="button"
          ...
          {phoneSecondary ? (
            <div className="grid grid-cols-2 gap-2">
              {[{ label: "Principal", value: phonePrimary }, { label: "Secondaire", value: phoneSecondary }].map((p) => (
                <button
                  key={p.value}
                  type="button"
                  onClick={() => { setSelectedPhone(p.value); setDialInput(p.value); }}
          ...
          <div className="grid grid-cols-3 gap-2">
            {KEYS.map((k) => (
              ...
              >
                <span className="text-base font-semibold leading-none text-zinc-800 dark:text-white">{k.digit}</span>
                <span className="mt-0.5 text-[8px] font-medium tracking-widest text-zinc-400">{k.sub || " "}</span>
              </button>
            ))}
          </div>
