...
export const CampaignPopoverForm = ({
  ...
  const layoutId: string = isEdit ? `campaign-edit-${campaign?.id}` : 'campaign-create'
  return (
    <MorphingPopover
      variants={{
    ...
    >
      ...
      <MorphingPopoverContent className="top-full right-0 mt-2 w-[480px] p-5 shadow-xl">
        <form ref={formRef} action={formAction} className="space-y-4">
          {isEdit ? <input type="hidden" name="campaignId" value={campaign?.id} /> : null}
          <motion.h3
            layoutId={`${layoutId}-label`}
            layout="position"
