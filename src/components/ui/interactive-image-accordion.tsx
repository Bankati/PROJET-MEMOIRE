...
const accordionItems: readonly AccordionItemData[] = [
  ...
  },
  {
    ...
    imageAlt: 'Intégrations WhatsApp pour le suivi',
  },
] as const
...
function AccordionItem({ item, isActive, onMouseEnter, index }: AccordionItemProps) {
  ...
  return (
    ...
    >
      ...
      <div
        className={`pointer-events-none absolute inset-0 rounded-2xl ring-2 transition-all duration-500 ring-inset ${
          isActive ? 'ring-lbs-blue/40' : 'ring-white/0'
        }`}
      />
      <span
        className={`absolute text-sm font-semibold whitespace-nowrap text-white drop-shadow-lg transition-all duration-500 ease-out ${
          isActive
      ...
      {isActive && (
        <motion.div
          className="absolute right-5 bottom-5 rounded-full bg-white/20 px-3 py-1 text-xs font-medium text-white backdrop-blur-sm"
