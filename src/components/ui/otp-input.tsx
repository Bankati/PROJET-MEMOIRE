    ...
    <div className="flex justify-center gap-3">
      {Array.from({ length }).map((_, index) => (
        <input
          ...
          disabled={disabled}
          autoFocus={index === 0}
          className="size-14 rounded-2xl border border-white/20 bg-white/10 text-center text-xl font-semibold text-white shadow-lg outline-none transition-all duration-200 placeholder:text-white/30 focus:border-white/50 focus:bg-white/20 focus:ring-2 focus:ring-white/20 disabled:opacity-50"
        />
      ))}
    </div>
  );
