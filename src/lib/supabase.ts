...
export const uploadRagDocument = async ({
  ...
  const { error } = await supabase.storage.from(RAG_DOCUMENTS_BUCKET).upload(filePath, fileBuffer, {
    contentType,
    upsert: false,
  })
  if (error) {
...
export const deleteRagDocument = async ({
  ...
  const { error } = await supabase.storage.from(RAG_DOCUMENTS_BUCKET).remove([storagePath])
  if (error) {
