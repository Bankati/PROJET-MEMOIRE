...
export default async function AdminExportPage({
  ...
  const totalCallsResult: Array<{ value: number }> = hasCampaigns
    ? await db.select({ value: count(callResults.id) }).from(callResults).where(callWhereClause)
    : [{ value: 0 }];
  const totalCallsCount: number = totalCallsResult[0]?.value ?? 0;
  const falseNumbersResult: Array<{ value: number }> = hasCampaigns
    ? await db.select({ value: count(callResults.id) }).from(callResults).where(and(callWhereClause, eq(callResults.outcome, "false_number")))
    : [{ value: 0 }];
  const falseNumbersCount: number = falseNumbersResult[0]?.value ?? 0;
  const whatsappResult: Array<{ value: number }> = hasCampaigns
    ? await db.select({ value: count(callResults.id) }).from(callResults).where(and(callWhereClause, eq(callResults.isWhatsappRedirected, true)))
    : [{ value: 0 }];
  const whatsappCount: number = whatsappResult[0]?.value ?? 0;
  const avgDurationResult: Array<{ value: number | string | null }> = hasCampaigns
    ? await db.select({ value: sql<number>`coalesce(round(avg(${callResults.durationSeconds})::numeric, 0), 0)` }).from(callResults).where(callWhereClause)
    : [{ value: 0 }];
  ...
  const selectedTitle: string = campaignFilter.length > 0
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Générez un rapport PDF de vos performances</p>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200/70 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-zinc-800 dark:text-white">Rapport — {selectedTitle}</h2>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">Période : {dateFromStr} au {dateToStr}</p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-xl border border-zinc-100 p-4 dark:border-white/10">
            ...
            <p className="mt-2 text-2xl font-bold text-zinc-900 dark:text-white">{totalCallsCount}</p>
          </div>
          <div className="rounded-xl border border-zinc-100 p-4 dark:border-white/10">
            <div className="flex items-center gap-2">
            ...
            <p className="mt-2 text-2xl font-bold text-zinc-900 dark:text-white">{falseNumbersCount}</p>
          ...
          <div className="rounded-xl border border-zinc-100 p-4 dark:border-white/10">
            ...
            <p className="mt-2 text-2xl font-bold text-zinc-900 dark:text-white">{formatDuration({ seconds: avgDuration })}</p>
          </div>
        </div>
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          <div className="rounded-xl border border-zinc-100 p-4 text-center dark:border-white/10">
            ...
            <p className="mt-1 text-3xl font-bold text-lbs-blue">{Math.max(0, totalCallsCount - falseNumbersCount - whatsappCount)}</p>
          </div>
          <div className="rounded-xl border border-zinc-100 p-4 text-center dark:border-white/10">
            <p className="text-sm font-medium text-zinc-600 dark:text-zinc-300">Taux de faux numéros</p>
        ...
        <div className="mt-6 rounded-xl border border-dashed border-zinc-300 p-8 text-center dark:border-white/15">
          ...
          <p className="text-sm font-medium text-zinc-500 dark:text-zinc-400">Le rapport PDF sera généré avec ces données.</p>
          <p className="mt-1 text-xs text-zinc-400">Le fichier inclura les KPI, graphiques et la mise en forme visuelle complète.</p>
        </div>
      </div>
    </div>
  );
