/**
 * Fiche d'appel admin — même interface 3 panneaux que l'agent.
 */
...
async function submitCallResult(formData: FormData): Promise<void> {
  ...
  redirect("/dashboard/admin/calls?notice=call_saved");
}

export default async function AdminCallDetailPage({ params }: PageProps): Promise<React.JSX.Element> {
  const user = await requireRole({ allowedRoles: ["admin"] });
  ...
  const [extraData] = await db
    .select({
      ...
      source: sql<string | null>`cast(${campaignContacts.source} as text)`,
      ccCreatedAt: campaignContacts.createdAt,
      ccNotes: campaignContacts.notes,
    })
    .from(campaignContacts)
    .innerJoin(contacts, eq(campaignContacts.contactId, contacts.id))
    .innerJoin(agentContactAssignments, eq(agentContactAssignments.campaignContactId, campaignContacts.id))
    .where(and(
  ...
  const contactMetadata: Record<string, unknown> = extraData?.metadataText ? JSON.parse(extraData.metadataText) : {};
  const contactSource = extraData?.source ?? null;
  const contactCcCreatedAt = extraData?.ccCreatedAt ?? null;
  ...
  const completedNum = typeof progress.completed === "number" ? progress.completed : Number(progress.completed);
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isCompleted ? (
            ...
            <span className="rounded-full bg-amber-100 px-3 py-1.5 text-xs font-medium text-amber-700 dark:bg-amber-500/15 dark:text-amber-300">En attente</span>
          )}
          <a
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-2xl border border-zinc-200/70 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
            <div className="mb-3 flex items-center justify-between">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Campagne active</p>
            ...
            {contact.campaignDetails ? (
              <p className="mt-1 text-xs text-zinc-400 dark:text-zinc-500 line-clamp-2">{contact.campaignDetails}</p>
            ) : null}
            <div className="mt-4 grid grid-cols-2 gap-2">
              <div className="rounded-xl bg-zinc-50 px-3 py-2 dark:bg-white/5">
                ...
                <p className="mt-0.5 text-xs font-semibold text-zinc-700 dark:text-zinc-200">{assignedDate}</p>
              </div>
              <div className="rounded-xl bg-zinc-50 px-3 py-2 dark:bg-white/5">
                <div className="flex items-center gap-1 text-[10px] uppercase tracking-wider text-zinc-400">
                  <Clock className="size-3" /> Statut
                </div>
                <p className={`mt-0.5 text-xs font-semibold ${isCompleted ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400"}`}>
            ...
            <div className="mt-4">
              ...
              <div className="h-2 w-full overflow-hidden rounded-full bg-zinc-100 dark:bg-white/10">
                <div className="h-full rounded-full bg-gradient-to-r from-[#244976] to-[#21416C] transition-all" style={{ width: `${progressPct}%` }} />
              </div>
            </div>
          </div>
          ...
          {/* Contact full info */}
          <div className="rounded-2xl border border-zinc-200/70 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
            <p className="mb-3 text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Contact</p>
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                {contact.schoolName ? <p className="text-xs text-zinc-500 dark:text-zinc-400">{contact.schoolName}</p> : null}
                {contact.city ? <p className="text-xs text-zinc-400 dark:text-zinc-500">{contact.city}</p> : null}
              </div>
            </div>
            <div className="space-y-3">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-zinc-800 dark:text-white">{contact.phonePrimary}</p>
                  {contact.phoneSecondary ? <p className="text-xs text-zinc-400 dark:text-zinc-500">{contact.phoneSecondary}</p> : null}
                </div>
              </div>
              {contact.email ? (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="break-all text-sm font-medium text-zinc-700 dark:text-zinc-200">{contact.email}</p>
                  </div>
                </div>
              ) : null}
              {contact.schoolName ? (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[10px] text-zinc-400">Établissement</p>
                    <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">{contact.schoolName}</p>
                  </div>
                </div>
              ) : null}
              {contact.city ? (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">{contact.city}</p>
                  </div>
                </div>
              ) : null}
              <div className="flex items-start gap-3">
                <div className="grid size-7 shrink-0 place-items-center rounded-lg bg-slate-50 dark:bg-slate-500/10">
                  <Upload className="size-3.5 text-slate-500 dark:text-slate-400" />
                </div>
                <div>
                  <p className="text-[10px] text-zinc-400">Source</p>
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">
                    {contactSource === "excel_import" ? "Import Excel"
                      : contactSource === "manual_form" ? "Formulaire manuel"
                      : contactSource === "campaign_reuse" ? "Réutilisation campagne"
                      : contactSource ?? "—"}
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                <div className="grid size-7 shrink-0 place-items-center rounded-lg bg-pink-50 dark:bg-pink-500/10">
                  <CalendarDays className="size-3.5 text-pink-500 dark:text-pink-400" />
                </div>
                <div>
                  <p className="text-[10px] text-zinc-400">Ajouté le</p>
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">
                    {contactCcCreatedAt?.toLocaleDateString("fr-FR", { day: "numeric", month: "short", year: "numeric" }) ?? "—"}
                  </p>
                </div>
              </div>
              {contactCcNotes ? (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">{contactCcNotes}</p>
                  </div>
                </div>
              ) : null}
              {contactMetadata && Object.keys(contactMetadata).length > 0 ? (
                Object.entries(contactMetadata).map(([key, value]) =>
                  value != null && value !== "" ? (
                    <div key={key} className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-[10px] text-zinc-400">{key.replace(/[_-]/g, " ").replace(/\b\w/g, (c) => c.toUpperCase())}</p>
                        <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">{String(value)}</p>
                      </div>
                    </div>
                  ) : null
                )
          ...
          {queueContacts.length > 0 ? (
            <div className="rounded-2xl border border-zinc-200/70 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
              <div className="mb-3 flex items-center justify-between">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">File d&apos;attente</p>
              ...
              <div className="space-y-1">
                {queueContacts.map((q, idx) => (
                  <Link
                    key={q.ccId}
                    href={`/dashboard/admin/calls/${q.ccId}`}
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-medium text-zinc-800 dark:text-white">{q.firstName} {q.lastName ?? ""}</p>
                      <p className="truncate text-xs text-zinc-400 dark:text-zinc-500">{q.schoolName ?? "—"}</p>
                    </div>
                    <span className="shrink-0 text-xs font-semibold text-zinc-300 dark:text-zinc-600">{idx + 1}</span>
                  </Link>
                ))}
              </div>
            </div>
          ...
          {isCompleted ? (
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-500/30 dark:bg-emerald-500/10">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-emerald-800 dark:text-emerald-300">Appel traité</p>
                  <p className="text-sm text-emerald-600 dark:text-emerald-400">Le résultat a été enregistré.</p>
                </div>
              </div>
            </div>
          ) : (
