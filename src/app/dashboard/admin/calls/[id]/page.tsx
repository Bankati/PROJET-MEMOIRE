...
async function submitCallResult(formData: FormData): Promise<void> {
  ...
  try {
    ...
    redirect('/dashboard/admin/calls?notice=error')
  }
...
export default async function AdminCallDetailPage({
  ...
  const [progressResult, queueContacts] = await Promise.all([
    db
      .select({
            ...
            <div className="space-y-3">
                ...
                <div>
                  ...
                  {contact.phoneSecondary ? (
                    <a
                      ...
                      className="hover:text-lbs-blue block text-xs text-zinc-400 dark:text-zinc-500"
                    >
                      {contact.phoneSecondary}
                    </a>
                  ) : null}
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">
                    {contactSource === 'excel_import'
                      ? 'Import Excel'
                      : contactSource === 'manual_form'
                        ? 'Formulaire manuel'
                        : contactSource === 'campaign_reuse'
                          ? 'Réutilisation campagne'
                          : (contactSource ?? '—')}
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-zinc-700 dark:text-zinc-200">
                    {contactCcCreatedAt?.toLocaleDateString('fr-FR', {
                      day: 'numeric',
                      month: 'short',
                      year: 'numeric',
                    }) ?? '—'}
