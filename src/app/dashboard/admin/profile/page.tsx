...
import { Key, Mail, Save, Shield, User, X } from 'lucide-react'
...
async function updateProfile(formData: FormData): Promise<void> {
  ...
  if (fullName.trim().length === 0) {
    redirect('/dashboard/admin/profile?notice=missing_fields')
    return
  }
  try {
    await db
    ...
    redirect('/dashboard/admin/profile?notice=error')
  }
  redirect('/dashboard/admin/profile?notice=profile_updated')
}
...
async function changePassword(formData: FormData): Promise<void> {
  ...
  if (newPassword.length < 6) {
    redirect('/dashboard/admin/profile?notice=password_too_short')
    return
  }
  if (newPassword !== confirmPassword) {
    redirect('/dashboard/admin/profile?notice=password_mismatch')
    return
  }
  ...
  if (
    ...
    !verifyPassword({ password: currentPassword, storedHash: currentUser[0].passwordHash })
  ) {
    redirect('/dashboard/admin/profile?notice=wrong_password')
    return
  }
  try {
    await db
  ...
  redirect('/dashboard/admin/profile?notice=password_updated')
}
  ...
  return (
    <div className="space-y-6">
      ...
      {currentNotice ? (
        ...
        >
          {currentNotice.text}
          <a href="/dashboard/admin/profile" className="ml-auto">
            <X className="size-4" />
          </a>
        </div>
      ) : null}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <div className="dark:bg-lbs-surface-dark rounded-2xl border border-zinc-200/70 bg-white p-6 text-center shadow-sm dark:border-white/10">
            ...
            <div className="mt-3 inline-flex items-center gap-1.5 rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-700 dark:bg-blue-500/15 dark:text-blue-300">
              <Shield className="size-3" />
              Administrateur
            </div>
            <p className="mt-4 text-xs text-zinc-400">
              Membre depuis le {profile.createdAt.toLocaleDateString('fr-FR')}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="dark:bg-lbs-surface-dark rounded-2xl border border-zinc-200/70 bg-white p-6 shadow-sm dark:border-white/10">
            ...
            <form action={changePassword} className="space-y-4">
              ...
              <button
                type="submit"
                className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 px-5 py-2.5 text-sm font-medium text-white shadow-sm transition hover:brightness-110"
              >
