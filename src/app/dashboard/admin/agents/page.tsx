...
async function createAgent(formData: FormData): Promise<void> {
  ...
  const campaign = campaignId.length > 0
    ? await db.select({ endsAt: campaigns.endsAt }).from(campaigns).where(and(eq(campaigns.id, campaignId), eq(campaigns.createdByAdminId, admin.id))).limit(1)
    : [];
  const expiresAt: Date | null = campaign.length > 0 && campaign[0].endsAt ? campaign[0].endsAt : null;
...
async function reactivateAgent(formData: FormData): Promise<void> {
  ...
  const campaign = await db.select({ endsAt: campaigns.endsAt }).from(campaigns).where(and(eq(campaigns.id, campaignId), eq(campaigns.createdByAdminId, admin.id))).limit(1);
  const expiresAt: Date | null = campaign.length > 0 && campaign[0].endsAt ? campaign[0].endsAt : null;
  await db.update(users).set({
    status: "active",
    campaignAccessExpiresAt: expiresAt,
...
export default async function AdminAgentsPage({
  ...
  const agentCallCounts = myAgents.length > 0
    ? await db
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400">Gérez vos agents et leurs accès aux campagnes</p>
        </div>
      ...
      {currentNotice ? (
        <div className={`flex items-center gap-2 rounded-xl border px-4 py-2.5 text-sm ${
          ...
          {currentNotice.text}
          <a href="/dashboard/admin/agents" className="ml-auto"><X className="size-4" /></a>
        </div>
      ) : null}
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-zinc-200/70 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
          ...
          <p className="mt-2 text-3xl font-bold text-zinc-900 dark:text-white">{myAgents.filter((a) => a.status === "active").length}</p>
        </div>
        <div className="rounded-2xl border border-zinc-200/70 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
          <div className="flex items-center justify-between">
          ...
          <p className="mt-2 text-3xl font-bold text-zinc-900 dark:text-white">{myAgents.filter((a) => a.status === "expired").length}</p>
        </div>
      </div>
      {reactivateId.length > 0 ? (
        <div className="rounded-2xl border border-zinc-200/70 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
          <h2 className="mb-4 text-lg font-semibold text-zinc-800 dark:text-white">Réactiver l&apos;agent pour une campagne</h2>
          <form action={reactivateAgent} className="space-y-4">
            <input type="hidden" name="agentId" value={reactivateId} />
            <div>
              <label className="mb-1 block text-sm font-medium text-zinc-700 dark:text-zinc-300">Campagne *</label>
              <select name="campaignId" required className="w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-sm text-zinc-800 outline-none transition focus:border-lbs-blue dark:border-white/15 dark:bg-[#0f1729] dark:text-white">
            ...
            <div className="flex items-center gap-3">
              ...
              <a href="/dashboard/admin/agents" className="rounded-xl border border-zinc-200 px-4 py-2.5 text-sm text-zinc-600 transition hover:bg-zinc-50 dark:border-white/15 dark:text-zinc-300 dark:hover:bg-white/10">Annuler</a>
            </div>
          </form>
        </div>
      ) : null}
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody>
                {myAgents.length === 0 ? (
                  ...
                  myAgents.map((agent) => (
                    <tr key={agent.id} className="border-b border-zinc-100 transition hover:bg-zinc-50/50 dark:border-white/5 dark:hover:bg-white/5">
                      <td className="px-5 py-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium text-zinc-800 dark:text-white">{agent.fullName}</p>
                            <p className="text-xs text-zinc-500 dark:text-zinc-400">{agent.email}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-5 py-3">
                        <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${statusColorMap[agent.status]}`}>
                          {statusIconMap[agent.status]}
                          {statusLabelMap[agent.status]}
                        </span>
                      </td>
                      <td className="px-5 py-3 font-medium text-zinc-800 dark:text-white">{callCountMap.get(agent.id) ?? 0}</td>
                        ...
                        <div className="inline-flex items-center gap-1">
                          {agent.status === "active" ? (
                            <form action={toggleAgentStatus} className="inline">
                              ...
                              <button type="submit" className="rounded-lg p-1.5 text-zinc-500 transition hover:bg-amber-50 hover:text-amber-600 dark:text-zinc-400 dark:hover:bg-amber-500/10" title="Désactiver">
                                <Power className="size-4" />
                              </button>
                            </form>
                          ) : (
                            <Link href={`/dashboard/admin/agents?reactivate=${agent.id}`} className="rounded-lg p-1.5 text-zinc-500 transition hover:bg-emerald-50 hover:text-emerald-600 dark:text-zinc-400 dark:hover:bg-emerald-500/10" title="Réactiver">
                              <RefreshCcw className="size-4" />
                            </Link>
                          )}
                        </div>
