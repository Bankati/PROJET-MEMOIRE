...
const buildCurvePoints = ({
  ...
  while (cursor <= endMonth) {
    ...
    points.push({
      ...
      campaigns: monthlyCampaigns.get(key) ?? 0,
    })
...
export default async function AnalyticsPage({
  ...
  const campaignConditions: ReturnType<typeof and>[] = [
    gte(campaigns.createdAt, dateFrom),
    lte(campaigns.createdAt, dateTo),
  ]
  if (campaignFilter.length > 0) {
  ...
  const prevConditions: ReturnType<typeof and>[] = [
    gte(callResults.createdAt, prevFrom),
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Courbes d&apos;évolution et indicateurs filtrables
        </p>
      </div>
      <AnalyticsFilters
      ...
      />
      {campaignFilter.length > 0 ? (
        <div className="border-lbs-blue/20 bg-lbs-blue/5 text-lbs-blue flex items-center gap-2 rounded-xl border px-4 py-2 text-sm dark:border-blue-400/30 dark:bg-blue-500/10 dark:text-blue-200">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div
            key={stat.label}
            className={`rounded-2xl bg-gradient-to-br ${stat.color} p-5 text-white shadow-lg transition-transform duration-300 hover:scale-[1.02]`}
          >
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-zinc-200/70 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-xs font-medium ${callsTrend >= 0 ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300' : 'bg-rose-100 text-rose-700 dark:bg-rose-500/15 dark:text-rose-300'}`}
          >
