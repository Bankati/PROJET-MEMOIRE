...
export default async function CampaignsPage({
  ...
  const minCalls: number = Number.isFinite(Number(minCallsStr))
    ? Math.max(0, Number(minCallsStr))
    : 0
  ...
  const adminFilterOptions: readonly { value: string; label: string }[] = adminOptions.map(
    (name) => ({
      value: name,
      label: name,
    })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-bold text-zinc-900 sm:text-2xl dark:text-white">Campagnes</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Vue complète de toutes les campagnes de la plateforme avec filtres avancés
        </p>
      </div>
      <CampaignsFilters
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="font-semibold text-zinc-800 dark:text-white">{row.title}</p>
                  <p className="text-xs text-zinc-500 dark:text-zinc-400">
                    {row.year} · {row.ownerAdminName}
                  </p>
                </div>
                <span
                  className={`inline-flex shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium ${statusColor(row.status)}`}
                >
                  {statusLabel(row.status)}
                </span>
              </div>
              <div className="mt-3 flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
                <span className="inline-flex items-center gap-1 rounded-full bg-blue-50 px-2.5 py-1 font-medium text-blue-700 dark:bg-blue-500/15 dark:text-blue-300">
                  {row.callCount} appel{row.callCount > 1 ? 's' : ''}
                </span>
              ...
              {filteredRows.length === 0 ? (
                ...
                filteredRows.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4">
                      <span
                        className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${statusColor(row.status)}`}
                      >
                        {statusLabel(row.status)}
                      </span>
                    </td>
