...
export default async function SuperAdminDashboardPage({
  ...
  const [activeAdminsCount, activeAgentsCount, activeCampaignsCount] = await Promise.all([
    db.select({ value: count(users.id) }).from(users).where(and(eq(users.role, "admin"), eq(users.status, "active"))).then((r) => r[0]?.value ?? 0),
    db.select({ value: count(users.id) }).from(users).where(and(eq(users.role, "agent"), eq(users.status, "active"))).then((r) => r[0]?.value ?? 0),
    db.select({ value: count(campaigns.id) }).from(campaigns).where(eq(campaigns.status, "active")).then((r) => r[0]?.value ?? 0),
  ]);

  const totalCallsCount = (await db.select({ value: count(callResults.id) }).from(callResults).where(callWhereClause))[0]?.value ?? 0;
  const falseNumbersCount = (await db.select({ value: count(callResults.id) }).from(callResults).where(and(callWhereClause, eq(callResults.outcome, "false_number"))))[0]?.value ?? 0;
  const whatsappCount = (await db.select({ value: count(callResults.id) }).from(callResults).where(and(callWhereClause, eq(callResults.isWhatsappRedirected, true))))[0]?.value ?? 0;
  const interestedCount = (await db.select({ value: count(callResults.id) }).from(callResults).where(and(callWhereClause, eq(callResults.outcome, "interested"))))[0]?.value ?? 0;

  const avgDurationResult = await db
    .select({ value: sql<number>`coalesce(round(avg(${callResults.durationSeconds})::numeric, 0), 0)` })
    .from(callResults).where(callWhereClause);
  const avgDuration = extractCount({ value: avgDurationResult[0]?.value ?? 0 });
  ...
  const prevCallsCount = (await db.select({ value: count(callResults.id) }).from(callResults).where(and(...prevConditions)))[0]?.value ?? 0;
  const callsTrend = prevCallsCount === 0 ? (totalCallsCount > 0 ? 100 : 0) : Math.round(((totalCallsCount - prevCallsCount) / prevCallsCount) * 100);

  const monthlyCallsResult = await db
    .select({
  ...
  const recentAdmins = await db
    ...
    .from(users).where(eq(users.role, "admin")).orderBy(desc(users.createdAt)).limit(5);
  ...
  const campaignTableData = await Promise.all(
    allCampaigns.map(async (camp) => {
      const [totalContacts, totalCalls, whatsappConv, falseNums, interested] = await Promise.all([
        db.select({ value: count(campaignContacts.id) }).from(campaignContacts).where(eq(campaignContacts.campaignId, camp.id)).then((r) => r[0]?.value ?? 0),
        db.select({ value: count(callResults.id) }).from(callResults).where(eq(callResults.campaignId, camp.id)).then((r) => r[0]?.value ?? 0),
        db.select({ value: count(callResults.id) }).from(callResults).where(and(eq(callResults.campaignId, camp.id), eq(callResults.isWhatsappRedirected, true))).then((r) => r[0]?.value ?? 0),
        db.select({ value: count(callResults.id) }).from(callResults).where(and(eq(callResults.campaignId, camp.id), eq(callResults.outcome, "false_number"))).then((r) => r[0]?.value ?? 0),
        db.select({ value: count(callResults.id) }).from(callResults).where(and(eq(callResults.campaignId, camp.id), eq(callResults.outcome, "interested"))).then((r) => r[0]?.value ?? 0),
      ]);
      const topAgents = await db
        ...
        .where(eq(callResults.campaignId, camp.id))
      ...
      return {
        ...
        totalContacts,
        totalCalls,
        whatsappConversions: whatsappConv,
        falseNumbers: falseNums,
        interestedCount: interested,
        topAgents: topAgents.map((a) => ({
          agentName: a.agentName,
          totalCalls: a.totalCalls,
          whatsappCount: extractCount({ value: a.whatsappCount }),
        })),
      };
    })
  );
  ...
  const statCards = [
    ...
    { label: "Faux numéros", value: `${falseNumbersCount}`, gradient: "from-violet-400 to-violet-500", n: falseNumbersCount, badge: `${falseRate}%` },
    { label: "WhatsApp envoyés", value: `${whatsappCount}`, gradient: "from-cyan-400 to-cyan-500", n: whatsappCount, badge: `${whatsappRate}%` },
    { label: "Intéressés", value: `${interestedCount}`, gradient: "from-emerald-400 to-emerald-500", n: interestedCount, badge: `${interestedRate}%` },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Dashboard & Statistiques</h1>
        <p className="text-sm text-zinc-500 dark:text-zinc-400">Vue d&apos;ensemble et indicateurs de la plateforme</p>
      </div>

      <DashboardFilters campaigns={campaignOptions} currentCampaign={campaignFilter} currentFrom={dateFromStr} currentTo={dateToStr} />
      ...
      {campaignFilter.length > 0 ? (
        <div className="flex items-center gap-2 rounded-xl border border-lbs-blue/20 bg-lbs-blue/5 px-4 py-2 text-sm text-lbs-blue dark:border-blue-400/30 dark:bg-blue-500/10 dark:text-blue-200">
          ...
          Filtré sur : <span className="font-medium">{selectedCampaignTitle}</span>
          <a href="/dashboard/super-admin" className="ml-auto text-xs underline opacity-70 hover:opacity-100">Réinitialiser</a>
        </div>
      ) : null}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Administrateurs actifs", value: activeAdminsCount, icon: <Crown className="size-4 text-amber-400" />, hint: "Comptes de supervision" },
          { label: "Campagnes actives", value: activeCampaignsCount, icon: <Globe className="size-4 text-blue-400" />, hint: "En cours" },
          { label: "Agents actifs", value: activeAgentsCount, icon: <Users className="size-4 text-emerald-400" />, hint: "Disponibles" },
          { label: "Volume d'appels", value: totalCallsCount, icon: <Phone className="size-4 text-violet-400" />, hint: `Du ${dateFromStr} au ${dateToStr}` },
        ].map((kpi) => (
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="rounded-2xl border border-zinc-200/70 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
          <p className="mb-3 text-sm font-medium text-zinc-500 dark:text-zinc-400">Indicateurs clés</p>
          <div className="space-y-3">
            {[
              { label: "Faux numéros", value: falseNumbersCount, rate: falseRate, icon: <PhoneMissed className="size-3.5 text-rose-400" />, color: "bg-rose-100 text-rose-700 dark:bg-rose-500/15 dark:text-rose-300" },
              { label: "WhatsApp", value: whatsappCount, rate: whatsappRate, icon: <Send className="size-3.5 text-emerald-400" />, color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/15 dark:text-emerald-300" },
              { label: "Intéressés", value: interestedCount, rate: interestedRate, icon: <TrendingUp className="size-3.5 text-blue-400" />, color: "bg-blue-100 text-blue-700 dark:bg-blue-500/15 dark:text-blue-300" },
              { label: "Durée moy.", value: formatDuration({ seconds: avgDuration }), rate: null, icon: <Phone className="size-3.5 text-violet-400" />, color: "bg-violet-100 text-violet-700 dark:bg-violet-500/15 dark:text-violet-300" },
            ].map((row) => (
              <div key={row.label} className="flex items-center gap-3 rounded-xl bg-zinc-50 px-3 py-2.5 dark:bg-white/5">
                {row.icon}
                ...
                {row.rate !== null ? <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${row.color}`}>{row.rate}%</span> : null}
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="rounded-2xl border border-zinc-200/70 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
          <h3 className="mb-4 flex items-center gap-2 text-sm font-semibold text-zinc-700 dark:text-zinc-200">
            <BarChart3 className="size-4 text-lbs-blue" />
            Détail mensuel
          </h3>
              ...
              <tbody>
                {curvePoints.map((point) => (
                  <tr key={point.label} className="border-b border-zinc-100 dark:border-white/5">
                    <td className="px-2 py-2.5 font-medium text-zinc-800 dark:text-zinc-200">{point.label}</td>
              ...
              {recentAdmins.map((admin, idx) => (
                <div key={admin.id} className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-zinc-800 dark:text-white">{admin.fullName}</p>
                    <p className="truncate text-xs text-zinc-500 dark:text-zinc-400">{admin.email}</p>
                  </div>
                  <p className="shrink-0 text-[11px] text-zinc-400">{formatTimeAgo({ date: admin.createdAt })}</p>
                </div>
              ))}
            </div>
          )}
