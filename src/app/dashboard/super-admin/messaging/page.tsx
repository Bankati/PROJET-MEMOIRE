...
const broadcastAction = async (formData: FormData): Promise<void> => {
  ...
  if (message.length < 8) {
    redirect("/dashboard/super-admin/messaging?notice=Le+message+doit+contenir+au+moins+8+caractères.&success=false");
    return;
  }
        ...
        <div className="rounded-2xl border border-zinc-200/70 bg-white shadow-sm dark:border-white/10 dark:bg-[#1a2332]">
          {history.length === 0 ? (
            <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
              ...
              <p className="text-sm text-zinc-400 dark:text-zinc-500">Aucun message envoyé pour l&apos;instant.</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead>
                  <tr className="border-b border-zinc-100 text-xs uppercase text-zinc-400 dark:border-white/10 dark:text-zinc-500">
                  ...
                  {history.map((msg) => (
                    ...
                    >
                      <td className="px-5 py-3 max-w-xs">
                        <p className="line-clamp-2 text-zinc-800 dark:text-zinc-100">{msg.message}</p>
                        <p className="mt-0.5 text-xs text-zinc-400 dark:text-zinc-500">par {msg.senderName}</p>
                      </td>
                      <td className="px-5 py-3 whitespace-nowrap">
                        <span className="inline-flex items-center gap-1 rounded-full bg-lbs-blue/10 px-2.5 py-0.5 text-xs font-medium text-lbs-blue dark:bg-blue-500/15 dark:text-blue-300">
                          {msg.recipientCount} admin{msg.recipientCount > 1 ? "s" : ""}
                        </span>
                      </td>
                      <td className="px-5 py-3 whitespace-nowrap text-xs text-zinc-400 dark:text-zinc-500">
