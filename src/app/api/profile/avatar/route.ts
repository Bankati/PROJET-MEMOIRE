...
export const POST = async (request: Request): Promise<Response> => {
  ...
  if (!ALLOWED_TYPES.has(file.type)) {
    return NextResponse.json(
      { ok: false, message: 'Format non supporté. Utilisez JPEG, PNG ou WebP.' },
      { status: 400 }
    )
  }
  if (file.size > MAX_SIZE_BYTES) {
    return NextResponse.json(
      { ok: false, message: 'Fichier trop volumineux (max 2 Mo).' },
      { status: 400 }
    )
  }
  ...
  if (!avatarUrl) {
    return NextResponse.json(
      {
        ok: false,
        message: "Erreur lors de l'upload. Vérifiez la configuration Supabase Storage.",
      },
      { status: 500 }
    )
  }
