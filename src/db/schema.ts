...
export const userStatusEnum = pgEnum('user_status', ['active', 'inactive', 'expired'])
...
export const campaignVisibilityEnum = pgEnum('campaign_visibility', ['public', 'private'])
export const campaignPermissionEnum = pgEnum('campaign_permission', ['owner', 'editor', 'viewer'])
...
export const campaignCollaborators = pgTable(
  ...
  (table) => ({
    ...
    campaignCollaboratorsPermissionIdx: index('campaign_collaborators_permission_idx').on(
      table.permission
    ),
  })
)
...
export const contacts = pgTable(
  ...
  (table) => ({
    contactsPhonePrimaryUniqueIdx: uniqueIndex('contacts_phone_primary_unique_idx').on(
      table.normalizedPhonePrimary
    ),
...
export const campaignContacts = pgTable(
  ...
  (table) => ({
    campaignContactsCampaignIdx: index('campaign_contacts_campaign_idx').on(table.campaignId),
    campaignContactsContactIdx: index('campaign_contacts_contact_idx').on(table.contactId),
...
export const agentContactAssignments = pgTable(
  ...
  (table) => ({
    agentAssignmentsAgentIdx: index('agent_assignments_agent_idx').on(table.agentId),
